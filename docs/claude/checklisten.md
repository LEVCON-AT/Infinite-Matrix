...
- [ ] **(3) Mutations** — `lib/<domain>.ts`: CRUD-Funktionen durch `runOptimisticInsert`/`-Update`/`-Delete`. Reads mit `mergeRows`/`putAll` + `getByWorkspace`/`getById`-Fallback. **`supabase.rpc(...)`-Calls zaehlen mit:** RPC im Live-Pfad, `buildOffline` synth-Row + direkter `from(table).insert()` im Replay. Multi-Step-RPCs als zwei Specs queuen (siehe `architektur.md` §4.1.1). **Schleichpfad-Risiko: leicht zu vergessen, weil `.rpc()` formal kein `.from().insert()` ist — funktional aber identisch.**
