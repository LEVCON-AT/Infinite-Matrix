...
**Status 2026-05-09 — V1 LIVE.** `AtomMarkerBar`-Component (Star + Eye Click-Toggle, 1.5s-Throttle pro Atom+Kind, Star-Counter rechts vom Icon, Self-Eye-Indicator) gewired in BoardView Card-Meta-Row + CardOverlay-Header (V2.A 2026-05-09) + TaskDetail-Header (V2.B 2026-05-09). `wsAtomMarkers`-Resource in `Workspace.tsx` (`fetchAtomMarkersForWorkspace`) + Realtime-Refetch ueber `atom_markers`-Slot. TaskDetail-Route hat eigene `wsAtomMarkers`-Resource (Direct-Open via `/w/<wid>/task/<tid>` ohne Workspace.tsx-Bundle). RLS filtert eye-Marker auf Owner. Konzept-Vorgabe „rechts in der Card-Toolbar" via `margin-left: auto` in `.atom-marker-bar`.

**V1-Scope:** Click-Toggle (Star setzen/entfernen, Eye setzen/entfernen) + Counter (nur Star) + 1.5s-Throttle. Render in BoardView-Cards (Meta-Row) + CardOverlay-Header + TaskDetail-Header. NodeTree-Atom-Rows + ChecklistPanel-Items folgen wenn die Card-Variants den `AtomMarkerBar` als Render-Hook bekommen.
